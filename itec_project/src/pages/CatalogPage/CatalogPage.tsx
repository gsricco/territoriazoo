...
const CatalogPage = ( { openFiltersMode, closeEditMode }: CatalogPagePropsType ) => {
  ...
  useEffect( () => {
    const brands = !!chosenBrands.length ? chosenBrands?.join() : null; // we don't add
    const subCategories = !!chosenSubCategories.length ? chosenSubCategories?.join() : null;
  ...
  return (
    ...
    >
      ...
      <div className={ style.title }>
        <h1>{ chosenAnimalTypeName ? `Каталог товаров: ${ chosenAnimalTypeName }` : 'Каталог товаров' }</h1>
      ...
      <div className={ style.mainBlock }>
        <div className={ style.sortingBlock }>
          <div className={ style.productsType }>
            ...
            { chosenAnimalTypeId ? <ProductTypesForm forBurger={ false }/> :
              <ChooseAnimalTypeForm forBurger={ false }/> }
